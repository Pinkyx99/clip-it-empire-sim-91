...
import { ArrowLeft, Play, Target, CheckCircle, XCircle } from 'lucide-react';

export const CapCutApp = () => {
  const { gameState, setCurrentApp, addClip, setClipCooldown } = useGameData();
  ...
  const handleClickTarget = () => {
    ...
    if (isInTargetZone) {
      ...
      setTimeout(() => {
        setCurrentApp('twitch');
      }, 2000);
    }
  };

  const handleFinishEditing = () => {
    setCurrentApp('tiktok');
  };
  ...
  return (
    <div className="p-4 pb-24">
      ...
      <div className="flex items-center space-x-4 mb-6">
        <button 
          onClick={() => setCurrentApp(currentClip ? 'tiktok' : 'twitch')}
          className="p-2 rounded-xl bg-muted hover:bg-muted/80 transition-colors"
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
